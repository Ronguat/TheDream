...
public:
	 ...
	 *
	 *  A separate namespace from the ability's own tags: the input is not the move. One press of
	 *  InputTag.Attack resolves to Light, Heavy or Charged depending on how long it is held.
	 *
	 *  Routed by tag rather than by an integer ID, so adding an ability is a content change. The
	 *  character calls AbilitySpecInputPressed/Released, which sets Spec.InputPressed and forwards
	 *  both edges to live instances -- what WaitInputRelease observes, and what the hold-to-Heavy
	 *  conversion needs.
	 */
	 ...
	 *
	 *  True for refusals that are temporary in the sense the player meant -- a committed swing, a
	 *  blocking tag -- and false for those temporary only in the sense that everything ends, where
	 *  replaying produces an action nobody is still asking for. Asked of the ability rather than
	 *  enumerated on the character, so the rule stays next to the flag that creates it.
	 */
	virtual bool ShouldBufferFailedInput(const FGameplayAbilityActorInfo* ActorInfo) const;

	/**
	 *  Whether a buffered press for this ability's input survives while an instance is already
	 *  running, and through the string's link window after it ends.
	 *
	 ...
	 *
	 *  It is per ability, not per branch, so returning true also holds a press through a heavy or
	 *  charged and lets a second commitment queue on an unresolved first -- see the trap.
	 */
	 ...
	 *
	 *  The buffer tick calls this on the active instance answering the same input; an attack in its
	 *  chain-open span ends early through the ordinary EndAbility funnel -- facing, tags, homing and
	 *  the lunge cleaning up as on a natural end -- letting the same tick's retry activate the next
	 *  swing. Returns true if it ended.
	 */
	 ...
	 *
	 *  The character plays RiseMontage (or RiseHardMontage) when a rise begins, which is right for
	 *  the auto-rise, the neutral stand and the block get-up. The dodge animates itself: a roll on a
	 *  normal knockdown, a kip-up on a hard one.
	 */
...
protected:
	 ...
	 *
	 *  Keyed to the airborne state rather than to having jumped, so it also covers walking off a
	 *  ledge. Off by default: an air attack is a legitimate thing to want later.
	 */
	 ...
	 *
	 *  The counterpart to bLocksMovement below: that flag says "I take movement while I run", this
	 *  one says "I may not start while someone else has it". Reads ATheDreamCharacter's
	 *  bAbilityMovementLocked directly rather than through a mirrored tag. Off by default.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Combat|Activation")
	 ...
	 *
	 *  Three phases, two answers: the jail refuses everything regardless of this flag, and so does
	 *  the rise, which is committed once started. Only the choice window between them consults it.
	 *  Off by default, so the down state refuses anything nobody has thought about; the four that
	 *  take it are the dodge, the guard, the jump and the get-up attack, each pricing its own rise.
	 *
	 *  Grade restrictions are not here. Hard knockdown removes the directional dodge and the free
	 *  stand, answered by the option itself against the grade it reads off the character.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Combat|Activation")
	 ...
	 *
	 *  Input, not movement. The ability's own displacement is unaffected: a lunge, a dash or a
	 *  knockback still moves the character. What stops is walking out of your own commitment.
	 *
	 *  Cleared in this class's EndAbility, where every exit path converges. Off by default.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Combat|Activation")
	 ...
	 *
	 *  For held *states* rather than actions: a guard interrupted by a dodge or a swing comes back
	 *  when that finishes and the button is still down. Opt-in, and only states should carry it.
	 *
	 *  Cannot resurrect an ability the game still forbids -- the re-attempt goes through
	 *  CanActivateAbility, so exhaustion, a broken guard or being airborne refuse it as they would
	 *  a fresh press.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Combat|Activation")
...
protected:
	 ...
	 *
	 *  The release is guarded on this rather than on bLocksMovement, because EndAbility runs on the
	 *  shared base for every ability: without it, any ability ending would hand movement back while
	 *  another still owned it.
	 */
	 ...
	 *  to: dodging at 30 stamina works, empties the bar and exhausts you.
	 *
	 ...
	 *  general hook, not a mechanism anything depends on.
	 *
	 *  Applied on cancellation too, deliberately, so being hit out of a defensive action can never
	 ...
	 *  play an in-place clip and the dodge's source clips have bEnableRootMotion off; the
	 *  enforcement warning lives at StartAttackMontage.
	 *
	 ...
	 *  @param StandoffCm        Target Lock's per-tick gate. 0 disables it, which is what the dodge
	 *                           passes: an evade has to be able to travel *past* people.
	 ...
	 *  the instant the obstruction leaves. Correct while a target may back away mid-attack, wrong
	 *  once a hit has landed -- killing a target removes its capsule, the gate opens on a corpse and
	 *  the attacker slides through where it was.
	 *
	 *  Terminal: nothing restarts a lunge within one activation, so this can only ever subtract from
	 *  the authored distance. Safe to call when no lunge is running, or twice.
	 */
	void StopLunge();
...
private:
	 ...
	 *  ending, a cancellation or a montage interrupt without passing through here.
	 */
	 ...
	 *  input is held -- so an undeduped line would emit at frame rate.
	 */
