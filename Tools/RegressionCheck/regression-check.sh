...
BAND_ELAPSED_MIN=0.010;   BAND_ELAPSED_MAX=0.035
...
elapsed_values() { grep -o "elapsed=[0-9.]*" "$SLICE" | cut -d= -f2; }

count_per_attack() { # count_per_attack <TAG-regex>; count of TAG per ACTIVATE
	local tag="$1"
	awk -v tag="$tag" '
		/^\[[0-9.]+\] ACTIVATE/ { if (started) print n; started=1; n=0; next }
		started && $0 ~ tag { n++ }
		END { if (started) print n }' "$SLICE"
}
...
run_s3() {
	...
	assert_count "EXHAUSTED/END paired" \
		"$(grep -c '^\[[0-9.]*\] EXHAUSTION END' "$SLICE")" \
		"$(grep -c '^\[[0-9.]*\] EXHAUSTED ' "$SLICE")"
