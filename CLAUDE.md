...
8. ~~**Input buffering.**~~ **Implemented and verified in play 2026-08-11 — not yet refined.** A press nothing can answer is kept for a short window and retried each frame; single slot, last press wins. The window is grace on *taps* — a held button never expires, which is what makes a buffered heavy or charged reachable at all. A recorded release replays at its true offset, so the tier survives the wait. The airborne dodge refusal deliberately does **not** buffer. **The feel is unrefined and the window is a starting guess**, pending rounds of subjective tuning; `InputBufferSeconds` on `ATDCombatCharacter` is authoritative for the live value, and `Docs/Combat-Decisions.md` carries a trap about sizing it before item 12 settles what an attack's recovery actually is.
