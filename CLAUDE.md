...
2. ~~**Dodge**, and the stamina economy that shipped with it.~~ **Done 2026-08-10.** `DodgeSeconds` 0.4; `AM_Dodge` is eight untrimmed `Dash_*` clips at a derived 1.833×.
3. ~~**The character becomes a sword-and-shield fighter**~~ — camera-relative facing, the props, and the `SwordShield` locomotion set. **Done 2026-08-11.** Two things it left behind:
...
6. **The sword-and-shield attack swap.** **Next up** as of 2026-08-11. New attack montages, the `Release Window` notify re-placed **by hand** (not scriptable), `ReleaseStartSeconds` updated per branch. The most expensive slice in the project — choose the light clips with the string (item 9) in mind, or those notifies get placed twice.
   ...
   - **Clip candidates: the ~0.7 s chain stages, `Attack7` favoured** for its three native combo stages, which is what item 9 wants. The measurements and the **unmeasured assumption behind the play-rate estimate** are in `Docs/Combat-Decisions.md` — read that entry before choosing, because the figures argue for previewing rather than substituting for one.
