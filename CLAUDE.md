...
- **Block** — **mechanics shipped and played 2026-08-14**; the rules are in Core Combat Rules above. Two things remain, neither blocking the other:
   ...
   - **Blockstun's animation.** Mechanics shipped 2026-08-14 and are in Core Combat Rules above; what is missing is that it does not *read*. **`AM_Blockstun` exists as of 2026-08-15, ~90% built by script and internally inconsistent** — its trap in `Docs/Combat-Decisions.md` carries the open-and-save it needs, and the option to bin it. Beyond that: **nothing plays it** (no montage property exists anywhere, so wiring is a header change and a rebuild), and **directional means four montages**, sections being unscriptable.
