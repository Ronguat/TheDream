...
**Name the asset, not the C++ class, and this is a correctness rule rather than a style one** (2026-08-14). A Blueprint CDO override shadows a C++ default silently, so a class is the authority only until someone touches a details panel. Four documented live values disagree with their headers today — the buffer, the regen pause, `DodgeSeconds` and `AimAssistMarginCm` — and in every case the *asset* is right and the header is a starting value nobody edits. Pointing at `ATDCombatCharacter` therefore lands a reader on a number that is not live. Write *"`BP_PlayerCharacter`'s CDO is authoritative, defaults in `ATDCombatCharacter`"*, which names both and says which wins.
