...
deferred for time on 2026-08-18, and the **bespoke windup pass** that wants them was folded into
**Knockdown & Oki** the same day, so both are picked up there.
...
- **Knockdown & Oki** *(from Stun's 2026-08-15 split — shared plumbing justified adjacency, not a mono-slice)* — knockdown, the 1.5 s default get-up, the three early get-up options and the get-up attack, plus the guard break's full-lockout state the trap defers here and **jump-as-ability**, which rides it. **Gained 2026-08-16:** the charged's **hard knockdown** grade, hitstun's movement lock (deferred beside the guard break's), and the get-up attack consuming the knockback mechanism Light String builds. **Gained 2026-08-18 — the bespoke windup pass folds in here** (the designer, who called the fit "a little unclean" and took it anyway): heavy and charged get their own clips, their windups become **blended transitions** rather than a rate freeze, and **coil is deprecated**. The adjacency is real — heavy and charged inflict knockdowns too — and it has to come before the rest, since Interplay would otherwise judge feel with both tiers still playing the light's clip. Spec in the decision log, 2026-08-18. **Also gained that day, and this slice is now the string's terminator:** the light string's **ender knocks down**, and today it *displaces* instead — `ApplyKnockbackToTarget` runs on every unblocked hit with no final gate, deliberately left alone because this slice replaces what the ender does to its victim wholesale. That also makes this slice **what widens `s4-360`**: the ender's displacement is why that scenario asserts the first burst only, and replacing it lifts the restriction. See the trap. **`SwordShield` has no get-up content whatsoever** — unfiltered search for `Rise|GetUp|StandUp|Recover|Wake|Prone|Ground|KnockDown|Knock|Fallen|Down` returned zero for the archetype (2026-08-10). It exists only in `DaggerCombatAnimationV1` (18: `Rise1`–`Rise9`, two variants each) and `Unarmed` (8, including the bundle's only explicit `KnockDown`). Knockdown recovery therefore needs a **cross-archetype migration** — raise it before the slice starts.
