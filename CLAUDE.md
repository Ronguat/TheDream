...
3. **The character becomes a sword-and-shield fighter** — camera-relative facing, the `SwordShield` locomotion set, and the sword and shield on the `weapon_r` / `weapon_l` bones. Built and verified in play as 3a → 3b → 3c.
   ...
   - **3b — the sword and shield**, attached to `weapon_r` / `weapon_l`. Both meshes already migrated.
