...
**Pick up at Block's three remainders** — the stance's state machine, blockstun, and the per-ability
regen pause. All three are listed under Block in Remaining; none blocks the others.

*(The five open checks filed by the 2026-08-14 audit are all discharged. What they established that
still matters has moved to where it is used: the shadowed-value rule above, the felt-numbers table
and the vertical band's arithmetic in `Docs/Combat-Decisions.md`.)*
...
- **Block** — **mechanics shipped and played 2026-08-14**; the rules are in Core Combat Rules above. Three things remain, none blocking each other:
   ...
   - **Blockstun** — the lockout a *successful* block imposes, deliberately its own pass. Duration is set by the attack blocked, so it wants the ladder's numbers. It reads as the four directional `Defense_Hit_*` clips (verified 2026-08-10; there are also four die-while-blocking variants). Distinct from `State.GuardBroken`, which is the penalty for a guard that *failed*.
   - **Attacks should pause stamina regen** for their whole duration plus a tail, and **that tail should be authored per ability** rather than shared — jump, dodge, block and now attack each want their own. Agreed 2026-08-14 from play, not yet built.
