...
6. **The sword-and-shield attack swap.** **Next up** as of 2026-08-11. New attack montages, the `Release Window` notify re-placed **by hand** (not scriptable), `ReleaseStartSeconds` updated per branch. The most expensive slice in the project — choose the light clips with the string (item 9) in mind, or those notifies get placed twice.
   - **The melee trace moves off `hand_r` and onto the weapon.** Agreed 2026-08-11. `TraceSocket` is legacy from unarmed prototyping and is already wrong — the sword has been in hand since 3b and contributes nothing to what an attack hits. Reach grows when it moves, so the `TraceRadius` values (45 / 55 / 65) were tuned against a fist and need re-judging. Reasoning and the shape question — a swept sphere fits a fist, a blade is a line — in `Docs/Combat-Decisions.md`.
