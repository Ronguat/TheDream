...
8. **Input buffering.** **Next up** — moved ahead of 4 and 5 on 2026-08-11. Until it exists, an input pressed during a committed action is simply dropped, which is indistinguishable from the action feeling unresponsive, so every timing verdict is confounded. That confounds item 5's tuning as much as item 9's strings. Pure C++, no content, no editor.
