...
**Length is a context cost, not a style preference** (the user, 2026-08-15). This project is
maximally authored, so a large volume of documentation is internalised *every* session before any
work starts — verbosity is paid for in tokens up front, every time. The prose style is not the
problem; what it costs per session is. Trim tails, not heads: a bolded lead clause is what makes a
