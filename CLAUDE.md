...
- **Lunge + Recovery** — authored attack displacement, and the punish window it gets tuned against. **One slice** (2026-08-12): each authors a phase of the attack that the animation currently decides by default, both expect per-tier clips to land while they are open, and Recovery's number is what every later defensive verdict is measured from.
  ...
  - **Recovery becomes an authored `RecoverySeconds` with a derived play rate**, exactly as windup and release already work. Shape decided 2026-08-12; it is the only remaining item whose shape was settled before the slice opened. The `Release Window` notify denotes release, windup is whatever precedes it and recovery whatever follows, so an attack becomes three durations and the animation warps to fit. Today every attack's recovery is simply whatever is left of its montage.
  - **Watch the montage blend-out**, which ends the ability when blending *starts* and so eats the tail of any authored recovery. Filed as a trap; read it before authoring a number.
