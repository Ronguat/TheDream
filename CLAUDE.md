...
8. ~~**Input buffering.**~~ **Done 2026-08-11**, verified in play. A press nothing can answer is kept for `InputBufferSeconds` (100 ms) and retried each frame; single slot, last press wins. The window is grace on *taps* — a held button never expires, which is what makes a buffered heavy or charged reachable at all. A recorded release replays at its true offset so the tier survives the wait. The airborne dodge refusal deliberately does **not** buffer. **Feel is not tuned**: 43% of buffered presses expired unfired in the first session, and whether the window should be 200 ms is open — see the trap in `Docs/Combat-Decisions.md` about sizing it against a recovery length item 12 has not settled.
