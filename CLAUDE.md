...
- **Heavy**: held past 150 ms, hits at 500 ms. Single hit. Safe on block, punishable on whiff. Knocks down. Higher range, moderate stamina damage. *Currently ~300 ms coil → damaging, more reactable than intended; deferred until the ladder is tuned as a whole.*
...
6. **The sword-and-shield attack swap.** **In progress** as of 2026-08-12; the light is built and tuned. `AM_LightAttack_01` plays V3 `Attack4_Stage1_Complete`, the notify is placed by hand at 0.3046 for 0.1483, and the ladder is **150 ms input boundary → hits at 200 ms → 150 ms of release**, with no play-rate discontinuity across the three phases. Three bugs were found and fixed getting there — see `Docs/Combat-Decisions.md`, 2026-08-12.
   **Regression pass done 2026-08-12: hit detection, facing, ability lifecycle, i-frames and the stamina economy all pass**, plus a clean log check. Attacking from altitude was untestable — nothing places the dummy below the player — so the wedges' vertical band is **unverified rather than verified**; it spans ±70 cm against a 96 cm capsule half-height, so it cannot currently exclude anyone standing and only matters on slopes or against a jump. **The three wedges are deliberately uniform** (150 cm / 60° / ±70, authored explicitly on all three branches) and get re-authored alongside item 13 once each attack has its own animation — reach and travel are one felt quantity, and two of the three tiers are still playing the light's clip. `RootMotionScale` is back to **1.0**: 3.0 was tried and abandoned because scaling only amplifies the animator's curve, which is what item 13 exists to replace. The dummy's spacing is approximate for the same reason and is accepted until Lunge exists.
   ...
   - **Facing locks for the release window, and fades into and out of the lock.** An actor-frame volume needs a stable actor frame, and facing otherwise *snaps* to the camera on any frame with movement input. Steering stays free through windup — which is deliberate, and keeps the whole cancellable portion of an attack steerable — and through recovery. **The two fades are deliberately asymmetrical:** going in, `FacingLockFadeSeconds` is boxed in by the commit→release gap (50 ms, ~12° of turn) because anything longer would still be moving when the hitbox appears; coming out, `FacingUnlockRecoveryFraction` spends half of recovery, because there is no deadline and play found 50 ms snappy. **Whoever takes facing away restores it in `EndAbility`**, where every exit path converges, never on the montage delegates — and via `EnsureFacingRestored`, which leaves an in-flight recovery fade alone rather than re-timing it.
