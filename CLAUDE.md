...
2. ~~**Dodge.**~~ **Done 2026-08-10**, verified in play including its animation: eight-way, i-frames for its full duration, cost applied rather than gated, cancels an attack before its commit checkpoint, refused while airborne, `DodgeSeconds` 0.4, `AM_Dodge` built from eight untrimmed `Dash_*` clips at a derived 1.833× so animation and duration match exactly. The **stamina economy** shipped with it and is verified too — regen, the post-action pause, exhaustion entering at 0 and releasing at full. **Only `Fw` and `Bw` are reachable until item 3.**
3. **The character becomes a sword-and-shield fighter.** Three things that only make sense together: camera-relative movement (the character faces the camera rather than its own input direction), the **full** `SwordShield` directional locomotion set, and the sword and shield attached to the `weapon_r` / `weapon_l` bones.
   - **This blocks any real test of the dodge.** `ResolveDodgeDirection` measures input against actor facing, and `bOrientRotationToMovement` means the actor already faces its input, so it resolves to `Fw` in steady state and `Bw` only via the stationary fallback. Across every dodge logged 2026-08-10, no other direction ever fired.
   - `ResolveDodgeDirection` needs **no code change** — it is already correct and merely degenerate. Locking facing to the camera lights up all eight directions as they are.
   - Locomotion cannot be deferred to item 6: camera-relative facing makes the character strafe and backpedal, and the stock ABP plays a forward run while moving sideways. The clips exist (`Walk` 192 / `Run` 192).
   - Done when: strafing reads correctly, and **all eight dodge sections fire**.
4. **Death, minimally.** Health reaches 0 and *nothing happens* — the character keeps acting and the bar stops meaning anything, which quietly poisons every damage observation after it. All the `Death`/`Respawn` code in the tree is Epic's `Variant_Combat` hierarchy, which our classes do not derive from; read it as reference, do not inherit it. Copy the `State.Exhausted` pattern instead — tag applied, abilities blocked, timed release — plus a debug auto-revive. Deliberately defers the design questions (respawn rules, whether death routes through knockdown, whether the dummy dies) to item 11.
5. **Dodge travel distance** — `AnimRootMotionTranslationScale`. After item 3 because judging it now means judging one direction and guessing about seven, and after item 4 because tuning it means dodging repeatedly beside an attacking dummy.
...
7. **Block, and the blockstun that arrives with it.** Blockstun disables offense and parry for a duration set by the attack blocked; it is the first *reactive* stun state and pulls in plumbing hitstun will also need. `SwordShield` has a full `Defense` start/loop/end set.
...
10. **Parry.** `SwordShield` has exactly one parry clip and no failed-parry variant; Katana's richer `Deflect` set is the wrong stance.
11. **Hit reaction, knockdown, and death's full treatment** — the stun family together, since they share state plumbing and the questions item 4 deferred get answered here. `SwordShieldAnimV3` has standalone `Hit` and `Death` clips; no get-up set in this archetype.
