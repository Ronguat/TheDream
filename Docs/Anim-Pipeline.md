...
route *(MCP, re-confirmed 2026-08-27)*, its sandbox refusing `import unreal`. **Remote execution is
on and verified**, and
...
`add_animation_notify_track` for the track *(Python, confirmed 2026-08-22, re-measured 2026-08-27 —
`AM_Attack` reads back one event, `MeleeWindow`)*. It does **not** dirty the package — `modify()` does,
...
being the route for a foreign-skeleton source. There is no IK Rig or IK Retargeter *toolset* *(MCP,
2026-08-21)*; Python carries 129 `IKRig`/`IKRetarget` classes *(confirmed 2026-08-27)*.
