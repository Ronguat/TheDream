...
| The fall looks rushed, or the knockdown reads too brief | **`KnockdownFallSeconds`** — the carry, **0.45 and provisional** since 2026-08-25. It sets the rate too, because the fitted window is divided by it. **Ceiling is `KnockdownLockoutSecondsNormal`**, the shorter lockout, and what must fit under it is the montage, `(played - from) / rate` | Chasing the remaining stiffness with this. Every value reproduces the clip's deceleration into the ground at a different speed — see the brief for the time curve, which is the actual fix. Also not `HitstunSeconds`: on a graded swing it keys only the attacker's movement return. |
| The body lands flat, or slides after visibly landing | **`KnockdownFallClipSeconds`** (0.8, where the clip lands) and **`KnockdownFallClipStartSeconds`** (0.35, where its reaction commits) — the fitted window between them, both **measured** off the pelvis | Moving one without recomputing the rate. The window divided by the carry *is* the rate, so **trimming either end slows playback and stretches whatever artifact is at the other end** — trimming the gather once inflated the flat tail from 81 ms to 144 ms. **Re-measure if the clip is swapped**; both are properties of `AS_SwordSwordAnimV3_Death_Bw_RM`, not of knockdowns. |
