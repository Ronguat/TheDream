...
fallback, `UTDChargedAttackAbility`'s swing → branch → fallback ladder (live values **0.75 / 0.65 /
0.75** on branches, **0.75 / 0.9725** on string swings), and `UTDGetUpAttackAbility`'s **computed**
`Windup + Release + Recovery − Elapsed`, the only one that is not authored. The lockout also carries
the recoil's duration and the tell's span, so a change to it moves three things at once.
**`s5-parry` is red on this today**: `BAND_PARRY_LOCKOUT_LIGHT` is light-scoped by its own name and
never covered the ender's 0.9725, and how to resolve that is a coverage question rather than a band
...
**The parried attacker's recoil tell shipped 2026-08-27**, closing the recoil gap named above: a `ParryLockout` state in the Locomotion machine on `AS_SwordSwordAnimV3_Hit_Fw_RM`, positioned from lockout progress by `UTDAnimTellTools::DriveParryLockoutTell` against `ParryLockoutTellPortionSeconds` (0.684, hitstun's seam). **The designer's ruling picked the clip**: blockstun reads guarded and hitstun reads vulnerable, and a parry lockout *is* a punish window, so the victim carries the vulnerable tell rather than a block reaction. What Polish still inherits here is **the look, not the construction** — and the open preview question of whether V3's parry pose reads consistently beside V1's held guard, which is unchanged. **The editor work is no longer human either** *(2026-08-24)*: `UTDStateMachineTools` creates states, transitions and rules from script, which is how the hitstun flinch was built. A search for a blockstun *asset* still finds nothing and still proves nothing -- the state is why.  **And one that is mechanical rather than presentational, filed here so it is not forgotten** *(the designer, 2026-08-24)*: **a guard break should count as a hit for the *attacker's* input freedom.** A blocked hit returns before the on-hit waiver, so the attacker stays pinned for the full recovery — the waiver's own comment keeps recovery as the punish window *"against whiffs and against blocks."* A **break** should not sit on that side of the line: it waives like a connect. **The defender side is unchanged.** **The knockdown fall wants a time curve, and this is the brief for it** *(2026-08-25)*. The fall is skipped past its gather and cut before its settle, and what remains is that the clip **decelerates into the ground** where gravity would accelerate — an animator's cushion that every constant rate reproduces faithfully. **The fix is to redistribute time along the trajectory**: roughly linear through the descent, accelerating through the last third so the landing arrives rather than settles. *Half the mechanism exists* — `KnockdownFallTimeMappingCurve` is already a property but shapes the **carry**, not the clip; the missing half is applying it to the montage playhead. **The technique is already proven in this codebase**: `Montage_Play` takes only a constant rate, so hold the rate at zero and drive the position per tick from the curve, exactly as `UTDAnimTellTools` does for the stun tells. **Carry and clip should share one curve** or the body slams down while sliding uniformly. **The curve has a second job, learned after the brief was first written**: not only flattening the cushion but **buying duration without slowing the landing**. The usable window is 0.45 s of clip, so at a constant rate 1.0x *is* 0.45 s and 0.6 s *is* 0.75x — duration and landing speed cannot both be had. Sketched at 0.75x over clip 0.35–0.65 and 1.0x over 0.65–0.80: 0.55 s total with the impact still at speed. **The risk is that the slowed topple reads floaty.** **The fall is parked at carry 0.6 with `KnockdownFallClipStartSeconds` at 0.0** — the offset mechanism is built and guarded but disabled, because alone it trades the gather for a longer flat tail; the measured 0.35 waits in the property header. **It owes the loop a rethink**: `run_s6`'s *"fall lands inside lockout"* computes the montage span as `(played - from) / rate`, which stops meaning anything once the rate is not constant. **~~`KnockdownFallSeconds` is provisional pending this~~ — SETTLED at 0.5 on 2026-08-28**, in the C++ default with no Blueprint override, the curve work it waited on being done — a curve changes which carry is right, so settling it first is wasted work. **The curve shipped 2026-08-28 and the trajectory followed it**: `AM_Knockdown` carries a baked
`MontageTimeStretchCurve` — the engine's own feature, which the brief's hand-built rate-zero driver
turned out not to be needed for — giving 1.514x through the gather, **1.000x through the fall** and
1.395-1.515x through the cushion. Judged *"fine, but subtle"* in play, which sent the slice to the
trajectory itself: `C_KnockdownArc` on the carry's `PathOffsetCurve` lifts the victim **60 cm** and
crashes them down, and `C_KnockdownCarry` paces the slide on the same time base. **What a curve
cannot do** is change the trajectory's *shape*; the measurement says the shape is fine and only the timing is wrong, which is why this precedes both reauthoring the clip and the physics option below. **If it does not land, the two remaining routes are** authoring a replacement in Cascadeur, or a `PhysicalAnimationComponent` blend — physics driven toward the animated pose, which keeps the capsule deterministic (and so keeps `KnockdownSpacingCm`'s coupling to each tier's reach intact) while the mesh gains give. A **full** ragdoll knockdown is refused: it cannot hold an authored 450, and 2026-08-16 already rejected impulses for knockback on exactly that ground. **The knockdown fall was signed off prototype-grade 2026-08-28**, across six rounds of the designer's eye: the montage's own `TimeStretchCurve` protects the fall at 1.000x while the gather and cushion absorb the compression; `C_KnockdownArc` on the carry's `PathOffsetCurve` lifts the **body** 15 cm as a *correction* derived from what the clip already supplies; `C_KnockdownCarry` runs constant to contact then hands a speed-matched skid across `KnockdownCarrySettleSeconds` 0.16; and the arc presses 4 cm into the floor so the source's release has nothing to snap. Regenerate all of it with `Tools/AnimPipeline/ue_knockdown_curves.py` — the assets alone do not carry the method. **`AM_Knockdown`'s blend-in went 0.25 to 0.05** in the same pass, which is what put the animator's authored gather on screen for the first time; that also bears on the *"transition into the down state reads abrupt"* complaint this brief inherited, and nobody has re-judged that complaint since. **Hitstun was re-phased and signed off prototype-grade 2026-08-28**, the same standard as the knockdown fall. Three faults, each found only after the one before it was fixed: the **carry** ran 0.2 s against a 0.55 s stun while the stagger clip back-loads, so the feet slid and the body then stumbled standing still; the **tell's playhead** was a linear map, so the stepping could not run at its own speed unless the absorb did too; and **rotation drift** made the whole thing unjudgeable from any angle but behind. Now: `C_KnockbackPacing` paces the carry by the clip's own travel, `C_HitstunTellPacing` gives the stepping **1.000x** while the absorb pays **1.424x**, and hitstun joined `IsFacingLocked`. Regenerate the two curves **together** with `Tools/AnimPipeline/ue_hitstun_curves.py`; the carry is derived against the tell's mapping and neither survives the other moving. **What is knowingly left**: a residual slide, described by the designer as *"sliding steps"* rather than the original slide-then-stumble. It is **structural, not a defect** — the knockback distance is variable (spacing 350 from wherever the victim is when hit) while the clip's step distance is fixed at **72.8 cm** inside the carry window, so the two can only agree at one distance and slide at every other. Pacing fixed *when* the feet move and cannot fix *how far*. Removing it needs a constant knockback distance (which deletes the spacing reset), a clip chosen to match the typical distance (clip selection, which Polish's windup pass is already stalled on), or stride warping, which this project has no system for. **The abruptness complaint is re-judged and closed** *(the designer, 2026-08-28)*: *"Seems fine to me. I don't know really what I'm looking for, but this is a visual polish thing anyway, so if I'm not seeing an issue, that sort of means there isn't one."* Looked at after the blend-in change, which is what put the clip's authored gather on screen. **Rotation drift during hitstun: fixed 2026-08-28**, and it was blocking judgement of the knockback re-phasing rather than merely queued — *"I can only ever see it from the back, because if I rotate the camera to observe the hitstun from the side, I see the visual artifacts."* `IsFacingLocked()` read `bDead || bKnockedDown || Super::` and **hitstun was simply missing**, which is why the artifact was hitstun-only: knockdown already had it. Adding `bInHitstun` is the whole change, and forced facing is unaffected because `TickForcedFacing` writes rotation directly rather than through the desired-rotation path. **The camera stays free**, so an aim held through a stun is not lost. **Not blockstun** — it leaves the defender free to walk and so free to turn. **The guard break and the parry lockout have the same shape and are not ruled on**; both currently drift. Verified by the designer in play; the dummy fixtures cannot see it, having no camera to drift with. The designer's observation, kept:  *"you can still rotate your character during hitstun... The force facing is reapplied each hit, but if someone is looking away from their assailant with their camera, their character's rotation drifts away in between each refresh."* `TickForcedFacing` clears `bForcedFacingActive` the instant the turn completes, so it is **a one-shot turn per hit rather than a hold for the stun**, and between hits nothing contests camera-relative facing. **What is knowingly left**: the descent still reads slightly **linear** through its middle. The body's fall is animation-dominated — roughly 80 of its 127 cm is the clip laying the body down at a near-constant rate — so the capsule arc cannot bend it, and the only lever is `AM_Knockdown`'s own stretch curve. Raised twice, signed off over twice, and recorded here rather than chased. **The target windups shipped 2026-08-25** — heavy **400 ms**, charged **800 ms**, with each tier's `HoldUntilSeconds` moved alongside so every runway and commit rate is preserved; `DodgeRecoverySeconds` retired to 0 in the same pass. **What clip selection inherits is a ceiling**: the heavy cannot grow past roughly **450 ms** without the light↔heavy gap outgrowing the parry window's usable margin and one press ceasing to cover both tiers, so there is about 50 ms of headroom to fit a clip into. The corrected reactability reference — measured from the light's arrival, not the coil — and the five audited relationships are in that day's entry. **The eight directional dodges are fitted to the dash as of 2026-08-28**, the designer's proposal from play and the get-up roll's construction reused: `DodgeClipSeconds` **0.667** fits only the travelling portion of each 0.8333 s section, dropping the rate 2.083x → **1.668** and leaving 0.167 s of settle outside the i-frames, cut short by any montage started over it. `AM_Dodge`'s blend-out went **0.05 → 0.10** in the same pass, it being the *whole* budget for a tail — GAS ends a montage with `Montage_Stop(BlendOut.GetBlendTime())` — and `AM_Dodge` was the only montage in the project not at 0.25. Judged *"a subtle but major improvement visually"* in play. **What is left here is one look question nobody has asked**: the seam is a single shared value because all eight clips agreed on it to the frame, but they differ by up to **100 ms** in where their feet settle, so whether any direction wants its own remains open. The coverage gap is a dated trap.
