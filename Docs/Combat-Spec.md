...
**`GA_Attack`'s `Branches` and `Positions` arrays and the character Blueprints' CDOs are authoritative for live
...
Reasoning for every choice is in `Docs/Decisions.md`; which knob to move when a verdict comes
...
entry in `Docs/Decisions.md`, which keeps the map, the scenario walks, and that date's
...
**Space is authored the same way, as of 2026-08-12.** Each tier also authors its damaging volume — reach, arc and a vertical band — as an `FTDAttackHitbox` on its branch, so range is a designed number rather than a property of the clip. **Every volume is measured to the target's body**, damage and aim wedge alike *(measured 2026-09-03)*: a reach of 150 lands on a capsule whose nearest point is within 150, an arc admits the angle the body subtends, a band the half-height.

**Displacement too, as of 2026-08-12** — two authored distances in centimetres, not a scale on the clip. A shared **base lunge** from the press to the light's input boundary, then a **per-cell lunge** from the commit checkpoint to the end of the release window. The coil carries neither, which is what keeps the tiers indistinguishable for as long as they must be.
...
- **The wedge's arc is the knob and means how wrong your aim may be; its reach is derived** — base lunge + branch lunge + damage reach + `AimAssistMarginCm`, the one authored number, and it is measured from where the swing began, so travel spends it *(2026-09-03)*. **The wedge must reach past hit range, and that gap is the design rather than slack**, or lock-on becomes a rangefinder. `bEnabled` turns a branch off; an arc of 0 does not. **The arc is a learnable constant** and stays static across the ladder *and* across string swings (2026-08-16) — an intuition for how much error is forgiven is only worth building if the answer does not change per attack.
...
**Input is buffered** (graduated from Input Buffer 2026-08-14): a single slot, last press wins. **The window is acceptance, not decay** *(2026-09-02, superseding "a held button never expires")*: a press counts if it was made within `InputBufferSeconds` of the moment it can act, measured from the **press**, the frame the deadline lands on included *(2026-09-03)*, and is discarded otherwise — holding buys no reach, and a buffered heavy is reachable by pressing inside the window rather than by holding through it. The airborne-attack and airborne-dodge refusals deliberately do **not** buffer. `BP_PlayerCharacter`'s CDO holds `InputBufferSeconds`.
...
- **The chain input window** *(2026-09-02)* — **400 ms wide, centred on the chain-out opening, and
  it closes inside recovery.** A press continues the string from **0.283 to 0.683** after the
  ...
  Two consequences are deliberate — the light's recovery is **67% unactionable** (133 ms before the
  span, 267 ms after it), and a chain delayed past `HitstunSeconds` 0.55 forfeits the string's
  guarantee, which is the bait the delay-and-bait layer is made of.
...
- Regen, the pause and exhaustion are orchestrated in C++ on `ATDCombatCharacter`, not by GameplayEffects — see `Docs/Decisions.md`.
