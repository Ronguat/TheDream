...
| `s7-death` | 0.1, **taps 3** | `Off`, `DebugAutoReviveSeconds` at its default **3.0** | death's own bookkeeping: `DEATH` fires (**n=0 fails**); every death lands at **exactly 0.0** health; every death has a `REVIVE`; `DEATH`→`REVIVE` is **3.000 ±60 ms**, tracking the debug affordance rather than a design value; and **zero `DAMAGED` while dead** — the killing blow shares the death's timestamp and is excluded by a strict comparison, so only a *later* hit counts |
