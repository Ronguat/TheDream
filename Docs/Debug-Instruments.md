...
`Tools/RegressionCheck/gen-matrix.py`, which `docs-check` re-runs to catch a fixture edit that forgot
the doc. What each row *asserts* is the labelled list its evaluator prints, in `regression_rows.py`
under the row's id; the rows the bash checker carried, and where each of their assertions moved when
it retired, are the disposition table in the 2026-09-03 legacy-audit entry.

**What follows describes the timer fixtures those rows ran on until 2026-09-03**, kept for the
lessons each paid for; the fixtures themselves are gone or converted, per that table.
