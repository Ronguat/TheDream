...
notifies cannot be read off the asset by the toolset (`AnimationLibrary` is a Python route, verified 2026-08-22
...
| `s6-knockdown` | 0.1, **taps 3** | `Off` | the ender's knockdown: every `KNOCKDOWN` reads `type=normal`; entry→rise **2.000 ±25 ms**; rise→stand **0.500 ±25 ms**; **zero `DAMAGED` between a knockdown and its rise** (floor invincibility, and it fails on n=0 knockdowns rather than passing vacuously); every rise `by=auto` |
| `s6-hard` | 0.22 | `Off` | the same spans from the other grade — `type=hard`, entry→rise 2.000, rise→stand 0.500. **The total is type-invariant by design**, which is why this cannot see the 1.5/0.5 split and `s6-stand` exists |
...
| `s7-death` | 0.1, **taps 3** | `Off`, `DebugAutoReviveSeconds` at its default **3.0** | death's own bookkeeping: `DEATH` fires (**n=0 fails**); every death lands at **exactly 0.0** health; every death has a `REVIVE`; `DEATH`→`REVIVE` is **3.000 ±60 ms**, tracking the debug affordance rather than a design value; and **zero `DAMAGED` while dead**, plus the impulse read off `DEATH SETTLE` at ragdoll teardown and banded **300-560 cm** against a measured 396-449 — the killing blow shares the death's timestamp and is excluded by a strict comparison, so only a *later* hit counts |
